...
MessageQueue.next() 👈 队头msg0 ---> msg1 ---> msg2 ---> msg3 ---> msg4 ---> 队尾msg5                                                
从队头开始取消息
						   -------------------------------------------------------
                                              👆
...
ThreadLocal的作用是为每个线程Thread对象分配一个类型为ThreadLocal.ThreadLocalMap的map容器，每个Thread对象都有自己的ThreadLocalMap容器，它存储的值只被当前线程读取和修改。而ThreadLocal相当于一个工具，可通过它提供的set()、get()、remove()方法往map中设置、获取、删除对象，这样就隐藏了ThreadLocalMap。
...
- `ThreadLocal`可以为线程提供句不变量，整个线程声明周期中任何位置都能使用该变量，避免线程中变量传参问题
...
- `Looper.loop()`不会阻塞主线程https://blog.csdn.net/wl1433289703/article/details/95633042
