...
Android中的上下文Context提供了各种功能，比如操作资源文件、获取应用信息、操作文件系统、权限、启动组件等等，可以说是无所不能、贯穿始终。协程中的上下文虽然也是贯穿整个协程，到处都有它的身影，但功能就没那么强大了，**可以把它简单的看成一种"数据结构"，像List、Map等一样，它的作用就是用于为协程保存各种数据的**，协程能用到的数据都用它来保存就好了。下面是协程上下文接口源码：
...
协程上下文是一组符加到协程中的对象，它是各种不同元素的"集合"，可以包括负责协程线程策略的元素`CoroutineDispatcher`、关于协程执行的元素`Job`、协程标识和名称`CoroutineName`、处理未被捕捉的异常`CoroutineExceptionHandler`等等。如果把协程看作一个轻量线程，协程上下文就像是线程的一组局部变量。协程上下文可以看作是map和set的混合体，因为get操作符可以通过Key获取一个Element对象，这就像是map；而Element本身又持有Key，又可以通过plus直接添加Element，这就像是Element的set集合，协程上下文是一种非常灵活的数据结构。

为了理解协程上下文CoroutineContext、上下文元素Element、上下文Key的关系，可以打一个比方：协程上下文就是动物园接口Zoo；上下文元素就是动物接口Animal，某种具体的动物类型就是上下文元素的子类也就是Animal的子类，比如猴子`Monkey`；而Key就是关动物笼子上贴的一个牌子记录动物的名字。如果你家里养了一只猴子(Monkey的实例)，那就可以说你家是一个动物园实例zoo1（上下文元素对象本身就是一个上下文对象），然后你又买了一条狗，你家又变成了一个新的动物园实例`zoo2=zoo1+dog`，如果再增加其他种类的动物，就通过+将新动物养到你家里，这时候又变成新的动物园。只是这个动物园比较特殊，每种动物只能养一个（每种动物只有一个笼子），比如上面的zoo2本来就有一只猴子，你跑到街上看到另一种猴子更加聪明把它买回去，这时候就只能把原来的那只猴子舍弃腾出笼子关新猴子。有一个小孩去你家要看狗，你来到一大堆笼子中间找到笼子贴的木牌（Key）上写着“狗”的笼子，然后指着笼子里的动物对小孩说，你要的狗在这里。

## 5.1 自定义Element

协程上下文虽然有一些具体的子类，但是它的实例对象却是无数种的，由于这种特别的数据结构特性，我们可以很方便的通过+操作符组合成各种不同的上下文对象。所有自定义上下文元素应该继承标准库的`kotlinx.coroutines.AbstractCoroutineContextElement`，建议使用以下风格。以下示例展示了存储当前用户名的授权上下文元素：

```kotlin

//AbstractCoroutineContextElement的源码
...
所有的协程对象都继承`AbstractCoroutine`，在完成前3部分组合后，构建协程对象时会将组合传递给协程构造函数中的`parentContext`参数，它表示从父环境中继承的上下文。而我们指的协程上下文是协程类中维护的成员变量`context`，可以看到最后context是`parentContext`+this(协程当作Job类型)的一个新组合对象。
...
上述示例中通过多种方式构建了在UI线程调度的协程，我们可以在闭包中编写UI代码，但是更新UI需要先请求数据，请求数据的接口函数通常会被定义为挂起函数，但是挂起操作并不会切换线程(下一篇文章[《suspend挂起函数&非阻塞挂起和恢复的实现原理》]()详解)，为了保证耗时、网络请求等操作的挂起函数是**全部**在子线程执行，可以通过`withContext(Dispatchers.IO)`主动切换线程：
